...
// Section Header — centered heading + subtitle, kept distinct from the opener by its secHead type.
const LAYOUT_SECTION_XML = LAYOUT_TITLE_XML.replace('type="title"', 'type="secHead"').replace(
  'name="Title Slide"',
  'name="Section Header"',
);
 ...
 * slide master, the Office theme, four layouts (Blank / Title Slide /
 * Title and Content / Section Header), and the chosen slide size. The deck has no slides
...
export const buildBlankDeck = (aspect: BlankDeckAspect): OpcPackage => {
  ...
  add('/ppt/slideLayouts/slideLayout4.xml', SLIDE_LAYOUT_CONTENT_TYPE, LAYOUT_SECTION_XML);
  ...
  addRels('/ppt/slideLayouts/_rels/slideLayout4.xml.rels', LAYOUT_RELS_XML);
